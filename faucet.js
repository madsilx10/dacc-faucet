...
async function main() {
  ...
  while (true) {
    ...
    if (mode === "4") burnAmount = await inputAmount("Burn");
    if (mode === "5") stakeAmount = await inputAmount("Stake");
    ...
    for (let i = 0; i < selected.length; i++) {
      ...
      // Input amount per wallet kalau mode 9
      if (mode === "9") {
        burnAmount = await inputAmount(`Burn wallet [${index}]`);
        stakeAmount = await inputAmount(`Stake wallet [${index}]`);
      }
