...
async function selectWallets(wallets, csrf, showBalance = false) {
  ...
  for (const w of wallets) {
    ...
    if (showBalance) {
      try {
        const cookieStr = await login(w.address, csrf);
        const profile = await getProfile(cookieStr, csrf);
        balanceStr = ` — ${profile.dacc_balance} DACC`;
      } catch (e) { balanceStr = " — ?"; }
    }
    console.log(`  [${w.index}] ${w.address}${balanceStr}`);
  }
...
async function selectMode() {
  ...
  console.log("  [8] Claim Badge");
  console.log("  [9] All (Faucet + Crate + Burn + Stake + Claim Fees + Badge)");

  return new Promise((resolve) => {
    rl.question("\nPilih mode (1-9): ", (answer) => {
      rl.close();
      const input = answer.trim();
      if (!["1","2","3","4","5","6","7","8","9"].includes(input)) { console.error("❌ Tidak valid."); process.exit(1); }
      resolve(input);
    });
  });
}
...
async function visitBadges(cookieStr, csrf) {
  return await axios.post(`${BASE_URL}/api/inception/visit/badges/`, {}, {
    headers: { "Content-Type": "application/json", "Cookie": cookieStr, "X-Csrftoken": csrf, "Origin": BASE_URL, "Referer": `${BASE_URL}/badges`, "User-Agent": UA, "Accept": "*/*" },
  });
}
...
async function doBadge(cookieStr, csrf) {
  process.stdout.write(`  🏅 Claim badge ... `);
  try {
    const res = await visitBadges(cookieStr, csrf);
    if (res.data.awarded) { console.log("✅ Badge baru didapat!"); }
    else { console.log("✅ Semua badge sudah diklaim."); }
    return res.data.awarded;
  } catch (err) { console.log(`❌ ${err.message}`); return false; }
}
...
async function doStatus(wallets, csrf) {
  ...
  for (const w of wallets) {
    ...
    try {
      ...
      console.log(`  🏅 Badge   : ${badgeCount} badge diklaim`);
    } catch (err) {
...
async function main() {
  console.log("🚀 DACChain Bot — Faucet, Crate, Burn, Stake, Claim Fees, Badge");
  ...
  while (true) {
    ...
    const showBalance = ["4","5","6","9"].includes(mode);
    const selected = await selectWallets(wallets, csrf, showBalance);

    let burnAmount = "0", stakeAmount = "0";
    if (mode === "4" || mode === "9") burnAmount = await inputAmount("Burn");
    if (mode === "5" || mode === "9") stakeAmount = await inputAmount("Stake");
    ...
    for (let i = 0; i < selected.length; i++) {
      ...
      if (mode === "8" || mode === "9") await doBadge(cookieStr, csrf);
