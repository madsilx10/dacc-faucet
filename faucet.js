...
const CRATE_LIMIT = 5;
const CRATE_DELAY = 30000; // 30 detik
...
async function doCrate(cookieStr, csrf) {
  ...
  for (let i = 1; i <= CRATE_LIMIT; i++) {
    ...
    try {
      ...
      const status = err.response?.status;
      console.log(`❌ [${status}] ${msg}`);
      if (msg && (msg.toLowerCase().includes("limit") || msg.toLowerCase().includes("already") || msg.toLowerCase().includes("cooldown"))) {
        console.log("  ⏰ Limit/cooldown, skip crate.");
        break;
      }
    }
    if (i < CRATE_LIMIT) {
      process.stdout.write(`  ⏳ Jeda 30 detik `);
      for (let s = 30; s > 0; s--) {
